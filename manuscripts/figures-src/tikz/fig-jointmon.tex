...
% Legend↔title gap uses ~30pt above axis-north (titles stay @1.015); a/b row
% sits higher so panel-c's raised legend clears a/b bottoms.
...
\begin{axis}[
  inspect default,
  name=ax_a,
  width=2.58in, height=1.18in, at={(0.66in,3.24in)},
  xmin=-0.02, xmax=1.05,
  ymin=-4.20, ymax=0.20,
  xlabel={rate},
...
\input{../out/jointmon-panel-a.tex}
\end{axis}
\panellabel{ax_a}{a}
...
\begin{axis}[
  inspect default,
  name=ax_b,
  width=2.58in, height=1.18in, at={(3.22in,3.24in)},
  xmin=-0.03, xmax=1.03,
  ymin=-0.03, ymax=1.03,
...
\addplot[gray!55, densely dotted, line width=0.7pt, forget plot, mark=none]
  coordinates {(0,0) (1,1)};
\node[font=\tickfont, text=black!45, rotate=38, anchor=west] at (axis cs:0.58,0.58) {chance};
\input{../out/jointmon-panel-b.tex}
\end{axis}
\panellabel{ax_b}{b}

% Shared a/b legend — above the title row (breathing room vs titles @1.015).
% South anchor @30pt clears title tops (~10pt) without dropping onto the spine.
\node[anchor=south, font=\tickfont, inner sep=0.5pt]
  at ($(ax_a.north)!0.5!(ax_b.north)+(0,30pt)$) {%
  filled\,=\,catch\quad open\,=\,FA\quad
  {\color{cG1}$\bullet$}~G1\quad
  {\color{cG2}$\bullet$}~G2\quad
  {\color{black!70}$\circ$}/{\color{black!70}$\bullet$}~PC\quad
  {\color{black!70}\tikz[baseline=-0.15ex]\draw[line width=0.45pt] (0,0) rectangle (1.05ex,1.05ex);}/{\color{black!70}\rule[0.15ex]{1.05ex}{1.05ex}}~DM%
};
...
\begin{axis}[
  inspect default,
  name=ax_c,
  width=5.14in, height=1.35in, at={(0.66in,1.30in)},
  xmin=-9.5, xmax=11.0,
  ymin=-9.5, ymax=9.5,
...
\input{../out/jointmon-panel-c.tex}
\end{axis}
\panellabel{ax_c}{c}
% Same 30pt south-anchor offset as a/b shared legend (title stays near spine).
\node[anchor=south, font=\tickfont, text=black!70, inner sep=0.5pt]
  at ([yshift=30pt] ax_c.north) {%
  {\color{cFalseAlarm}$\bullet$}~G1-exceeding accepted ($\alpha_{\mathrm{miss}}=0.10$)\quad
  {\color{black!55}$\bullet$}~certified non-exceeding%
};
...
\begin{axis}[
  inspect default,
  name=ax_d,
  width=5.14in, height=1.15in, at={(0.66in,0.02in)},
...
\input{../out/jointmon-panel-d.tex}
\end{axis}
\panellabel{ax_d}{d}
