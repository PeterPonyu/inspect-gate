...
%   legend @ 1.16  — horizontal row, just above the title/letter row (was 1.30;
%                    the old value floated c/d legends into a large mid-figure void)
...
% Panel a: MVTec tiles
% a/b at 2.72in clears c/d legend chrome (~2.73 south @1.16) with ~0.25in
% for rotated x-labels + gutter (was 3.42in / legend@1.30 → large mid void).
\begin{axis}[
  inspect default,
  name=axa, width=2.55in, height=1.15in, at={(0.55in,2.72in)},
...
\begin{axis}[
  inspect default,
  name=axb, width=1.70in, height=1.15in, at={(3.55in,2.72in)},
...
% Shared tile legend — tight band just above a/b titles (tracks a/b drop).
\path (axa.north east) -- (axb.north west) coordinate[midway] (abmid);
\node[anchor=south, font=\tickfont, inner sep=1pt] at ([yshift=16pt]abmid) {%
